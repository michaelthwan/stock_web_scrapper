...
def fetch_one_row(code, url):
    ...
    股票價格記錄 = parse_table(all_tables[1])
    股票統計數據 = parse_table(all_tables[2])
    股息和拆股 = parse_table(all_tables[3])
    會計年度 = parse_table(all_tables[4])
    盈利能力 = parse_table(all_tables[5])
    管理效益 = parse_table(all_tables[6])
    損益表 = parse_table(all_tables[7])
    資產負債表 = parse_table(all_tables[8])
    現金流量表 = parse_table(all_tables[9])
...
if __name__ == '__main__':
    watched_stocks = [836, 178, 23, 5, 322, 2388, 2600, 52, 4, 590, 1929, 1668, 11, 386, 3988, 1088, 939, 1398, 2378, 1211, 700, 1169, 1, 168, 1928, 914, 1044, 405, 2319, 151, 762, 62, 6, 2638, 435,
                      16, 2, 315, 2380, 883, 2628, 270, 12, 1972, 144, 3328, 388, 941, 17, 669, 267, 101, 83, 823, 1038, 3, 66, 3331, 345, 808, 1199, 293, 87, 6823, 19, 548, 2800]
